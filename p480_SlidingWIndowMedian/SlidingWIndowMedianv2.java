package p480_SlidingWIndowMedian;
import java.util.Arrays;

public class SlidingWIndowMedianv2 {
    public static void main(String[] args) {
        System.out.print("[");
        SlidingWIndowMedianv2 instance = new SlidingWIndowMedianv2();
    ...
    public double[] medianSlidingWindow(int[] nums, int k) {
        ...
        for(int i = 0; i <= nums.length - k; i++) {
            ...
            for(int j = 0; j < k; j++) {auxArray[j] = nums[i + j];}
            Arrays.sort(auxArray);
            if (!evenWindow) {result[contResult] = auxArray[k / 2];}
            else {result[contResult] = (auxArray[k / 2] + auxArray[k / 2 - 1]) / 2.0;}
            contResult++;
        }
        return result;
    }
